...
class BaseSmpTest(Test):
    ...
    def get_maxcpus_limit(self, machine, accel):
        self.vm.add_args('-S', '-machine', '%s,accel=%s' % (machine, accel))
        self.vm.launch()
        maxcpus_limit = None
        for entry in self.vm.command('query-machines'):
    ...
    def launch_and_check(self, expect_cores=1, expect_threads=1,
        ...
        if self.vm_base_args:
            self.vm._args.clear()
            self.vm.add_args(*self.vm_base_args)
...
class  TestPPC64(BaseSmpTest):
    """
    :avocado: enable
    :avocado: tags=ppc64
    """
    def test_with_pseries_tcg(self):
        """
        :avocado: tags=pseries,tcg
        """
        ...
        self.launch_and_check(cpus=(maxcpus_limit - 1), maxcpus=maxcpus_limit,
                              expect_cores=maxcpus_limit,
                              expect_sockets=0,
                              expect_threads=0)

        self.launch_and_check(cpus=maxcpus_limit,
                              expect_cores=maxcpus_limit,
                              expect_sockets=0,
                              expect_threads=0)

        self.launch_and_check_fail(cpus=(maxcpus_limit + 1))

class TestX86(BaseSmpTest):
    """
    :avocado: enable
    :avocado: tags=x86_64
    """

    def test_with_pc_kvm(self):
        """
        :avocado: tags=pc,kvm
        """
    ...
    def test_with_q35_kvm(self):
        """
        :avocado: tags=q35,kvm
        """
        self.vm_base_args = ['-cpu', 'host', '-machine', "q35,accel=kvm,kernel-irqchip=split",
        ...
        self.launch_and_check(cpus=(maxcpus_limit - 1),
                              maxcpus=maxcpus_limit,
                              expect_sockets=maxcpus_limit)
