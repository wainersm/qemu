...
class Test(avocado.Test):
    ...
    def setUp(self):
        self._vms = {}
        # Some tests tags should be mapped to VM argumments
        self._tag_to_vm_args = []
        ...
        # Handle the 'accel' tag if present
        #
        ...
        if self.accel:
            ...
            if self.accel == 'kvm':
                if kvm_available(self.arch, self.qemu_bin):
                    self._tag_to_vm_args.append('-enable-kvm')
                    avail = True
            elif self.accel == 'tcg':
                if tcg_available(self.qemu_bin):
                    self._tag_to_vm_args.extend(['--accel', 'tcg'])
                    avail = True
            else:
    ...
    def _new_vm(self, *args):
        ...
        if args:
            vm.add_args(*args)
        # Add args mapped from tags
        if self._tag_to_vm_args:
            vm.add_args(*self._tag_to_vm_args)
        return vm
